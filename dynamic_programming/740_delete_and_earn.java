...
class Solution {
    ...
    public int deleteAndEarn(int[] nums) {
        // if we sort the array, we do not need to delete elements smaller than
        // nums[idx] (ie nums[idx] - 1) because they are already computed
        // and saved in memo, we only need to delete nums[idx] + 1 and we can do this
        // simply by skipping them since the array is sorted
        Arrays.sort(nums);
